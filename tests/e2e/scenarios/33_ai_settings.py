        ...
        _eval_value(
            ...
            f"""(() => {{
                ...
                return {{
                    ...
                    authText: auth ? auth.textContent : "",
    ...
    try:
            ...
            ctx.expect(
                bool(
                    _poll(
                        ...
                        and "Wird geladen"
                        not in (
                            ctx.api.dom("#ai-provider-list").get("textContent") or ""
                        ),
                    )
                ),
        ...
        with ctx.step("Schluessel nur als Passwort setzen und Status pruefen"):
            ...
            state = _poll(
                ...
                lambda: (
                    current
                    if "hinterlegt" in (current := _custom_state(ctx)).get(
                        "authText", ""
                    )
                    else None
                ),
            )
            ctx.expect(bool(state), "Auth-Status wechselte nicht auf hinterlegt")
        ...
        with ctx.step("Aktiver Custom-Provider erscheint im Modelle-Tab"):
            ...
            ctx.expect(
                model_state.get("fetchText") == "Modelle abrufen",
