...
impl AiConfigService {
    pub fn load() -> Self {
        Self::load_from(persist::config_file("ai.json"))
    }

    pub fn load_from(path: PathBuf) -> Self {
        Self {
            data: persist::load_json(&path),
            path,
        }
    }
    ...
    pub fn recent_languages_set(&mut self, languages: Vec<String>) -> Result<(), AiConfigError> {
        if self.data.translate.recent_languages == languages {
            return Ok(());
        }
        self.data.translate.recent_languages = languages;
        self.save()
    }
...
mod tests {
    ...
    fn config_round_trip_and_partial_migration() {
        ...
        service
            .recent_languages_set(vec!["en".to_string(), "fr".to_string()])
            .unwrap();
        let reloaded = AiConfigService::load_from(path).data();
        assert_eq!(vec!["en", "fr"], reloaded.translate.recent_languages);
