...
pub fn asset_add_in(
    ...
    if package_dir.join("theme.json").is_file() {
        ...
        let tmp = tempfile::Builder::new()
            ...
            .map_err(|error| format!("failed to create tempfile: {error}"))?;
        fs::write(tmp.path(), bytes).map_err(|error| format!("failed to write asset: {error}"))?;
        tmp.persist(&target)
            .map_err(|error| format!("failed to rename asset: {error}"))?;
        ...
        Err(format!(
            "directory theme '{id}' does not exist; assets can only be added to directory themes"
        ))
    }
}
...
pub fn asset_remove_in(id: &str, filename: &str, themes_dir: &Path) -> Result<(), String> {
    ...
    match fs::remove_file(&path) {
        ...
        Err(error) => Err(format!("cannot delete asset '{}': {error}", path.display())),
    }
}
...
fn create_in(id: &str, parts: &ThemeParts, themes_dir: &Path) -> Result<ThemePackage, String> {
    ...
    if id_occupied(id, themes_dir) {
        return Err(format!("theme ID '{id}' is already taken"));
    }
...
fn delete_in(id: &str, themes_dir: &Path) -> Result<(), String> {
    ...
    if package.source == ThemeSource::Builtin {
        return Err(format!("builtin theme '{id}' cannot be deleted"));
    }
    ...
    if package_dir.is_dir() {
        fs::remove_dir_all(&package_dir).map_err(|error| {
            format!(
                "cannot delete theme directory '{}': {error}",
                package_dir.display()
            )
        })?;
    }
...
fn clone_in(source_id: &str, new_id: &str, themes_dir: &Path) -> Result<ThemePackage, String> {
    ...
    if id_occupied(new_id, themes_dir) {
        return Err(format!("theme ID '{new_id}' is already taken"));
    }
    let source = super::package_in(source_id, themes_dir)
        .ok_or_else(|| format!("unknown theme: '{source_id}'"))?;
    if source.content_css.is_empty() {
        return Err(format!("theme '{source_id}' cannot be duplicated"));
    }
...
pub(crate) fn write_parts_in(
    ...
    let staged = tempfile::Builder::new()
        ...
        .map_err(|error| format!("cannot create temporary theme directory: {error}"))?;
    ...
    if target.exists() {
        ...
        return Err(format!(
            "cannot publish theme directory '{}': {error}",
            target.display()
        ));
    }

    super::package::load_package(id, themes_dir)
        .ok_or_else(|| format!("cannot load saved theme '{id}'"))
}
...
fn write_staged(
    ...
    let manifest = serde_json::to_vec_pretty(&parts.manifest)
        .map_err(|error| format!("cannot serialize theme manifest: {error}"))?;
    fs::write(staged.join("theme.json"), manifest)
        .map_err(|error| format!("cannot write theme manifest: {error}"))?;
    fs::write(staged.join(CONTENT_CSS_FILE), &parts.content_css)
        .map_err(|error| format!("cannot write theme CSS: {error}"))?;
...
fn write_optional(dir: &Path, filename: &str, content: Option<&str>) -> Result<(), String> {
    ...
    fs::write(dir.join(filename), content)
        .map_err(|error| format!("cannot write theme file '{filename}': {error}"))
}
...
fn replace_directory(
    ...
    if !target.is_dir() {
        ...
        return Err(format!(
            "theme target '{}' is not a directory",
            target.display()
        ));
    }
    let backup = tempfile::Builder::new()
        ...
        .map_err(|error| {
            let _ = fs::remove_dir_all(staged);
            format!("cannot create theme backup: {error}")
        })?;
    let old = backup.path().join("old");
    if let Err(error) = fs::rename(target, &old) {
        let _ = fs::remove_dir_all(staged);
        return Err(format!("cannot back up existing theme: {error}"));
    }
    if let Err(error) = fs::rename(staged, target) {
        let rollback = fs::rename(&old, target);
        let _ = fs::remove_dir_all(staged);
        return match rollback {
            Ok(()) => Err(format!("cannot publish theme update: {error}")),
            Err(rollback_error) => Err(format!(
                "theme update failed ({error}); rollback failed: {rollback_error}"
            )),
        };
    }
    Ok(())
...
fn copy_directory(source: &Path, target: &Path) -> Result<(), String> {
    ...
    for entry in
        fs::read_dir(source).map_err(|error| format!("cannot read asset directory: {error}"))?
    {
        ...
        if file_type.is_dir() {
            ...
            fs::copy(entry.path(), destination)
                .map_err(|error| format!("cannot copy asset: {error}"))?;
        }
    }
    Ok(())
}
...
fn remove_legacy_files(id: &str, themes_dir: &Path) -> Result<(), String> {
    for suffix in [".css", ".dark.css", ".page.css"] {
        ...
        match fs::remove_file(&path) {
            ...
            Err(error) => {
                return Err(format!(
                    "cannot delete legacy theme file '{}': {error}",
                    path.display()
                ));
            }
        }
...
fn validate_writable_id(id: &str) -> Result<(), String> {
    ...
    if builtin::IDS.contains(&id) {
        Err(format!("builtin theme '{id}' cannot be modified"))
    } else {
        Ok(())
    }
}
